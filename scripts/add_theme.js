...
const themeScript = `
  <script>
    (function() {
      try {
        var theme = localStorage.getItem('FOLIX_THEME');
        if (!theme) {
...
const themeToggle = `<div class="nav__right">
...
for (const file of files) {
  ...
  // Add script before </head> if not exists
  if (!content.includes('FOLIX_THEME')) {
    content = content.replace('</head>', themeScript);
  }

  // Add toggle button to nav__right
  if (!content.includes('id="themeToggle"')) {
    content = content.replace('<div class="nav__right">', themeToggle);
  }
